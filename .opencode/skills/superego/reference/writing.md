...
If this project has superego configured (check for `.superego/` directory), you can invoke deeper review at milestones:

- **Switch to writing prompt:** `$superego prompt switch writing`
- **Review draft:** `$superego review <file>` (uses current prompt - will be writing prompt after switch)
- **Evaluate conversation:** `$superego` (advisory feedback on approach)

The writing prompt provides structured evaluation for content. Your role is conversational advisory during the creation process.
